...
![Element Toolbar](img/toolbar.png)
...
By default, all Toolbar Plugins will register for every Element. To specify the order and/or omit some plugins, modify an Element’s [Option Set](themer/cmp/option_sets.md) setting to either include or exclude specific Fred Plugins with a `plugins-include` or `plugins-exclude` attribute.
...
Include the JS file containing the init function using [includes](modx_events.md#includes) and registering the Plugin using [`beforeRender`](modx_events.md#beforerender).
