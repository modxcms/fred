...
switch ($modx->event->name) {
    case 'OnDocFormPrerender':
        if (!empty($resource) && !empty($fred->getTheme($resource->template))) {
            if (in_array($resource->class_key, $disabledClassKeys)) {
                return;
            }
    ...
    case 'OnWebPagePrerender':
        ...
        if (!empty($theme)) {
            ...
            if (!($modx->user->hasSessionContext('mgr') || $modx->user->hasSessionContext($modx->resource->context_key))) {
                return;
            }
            if (!$modx->hasPermission('fred')) {
                return;
            }

            // Additional check to load unrender resource class key
            $checkSym = $modx->getObject('modResource', $modx->resource->id);
            if (in_array($checkSym->class_key, $disabledClassKeys)) {
                return;
            }
    ...
    case 'OnBeforeDocFormSave':
        ...
        if (empty($fred->getTheme($resource->template))) {
            return;
        }
