...
class BlueprintsCreateBlueprint extends Endpoint
{
    function process()
    {
        ...
        $categoryObject = $this->modx->getObject(FredBlueprintCategory::class, ['id' => $category]);
        if (!$categoryObject) {
            return $this->failure($this->modx->lexicon('fred.fe.err.blueprints_ns_category'), ['category' => $this->modx->lexicon('fred.fe.err.blueprints_ns_category')]);
        }
        ...
        if (empty($rank)) {
            $c = $this->modx->newQuery(FredBlueprint::class);
            $c->where([
                'category' => $category
            ]);
            $c->sortby('rank', 'desc');
            ...
            $lastRecord = $this->modx->getIterator(FredBlueprint::class, $c);
            $rank = 1;
        ...
        $blueprint = $this->modx->newObject(FredBlueprint::class);
        ...
        if ($saved === true) {
            ...
            foreach ($templates as $template) {
                $blueprintAccess = $this->modx->newObject(FredBlueprintTemplateAccess::class);
