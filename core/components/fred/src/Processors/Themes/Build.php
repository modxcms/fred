    ...
    private function build($name, $version, $release, $themeId, $extractTpl)
    {
        ...
        if (is_dir($themeFolderPath) && is_readable($themeFolderPath)) {
            $vehicle = $builder->createVehicle([
                ...
                "vehicle_class" => FredFileVehicle::class
            ]);
            $vehicle->validate('php', [
                'source' => $this->fred->getOption('buildHelpers') . 'halt.validator.php'
            ]);
        ...
        foreach ($categories as $category) {
            ...
            $categoryObject = $this->modx->getObject(modCategory::class, ['category' => $category]);
            if ($categoryObject) {
        ...
        $namespace = $this->modx->getObject(modNamespace::class, ['name' => $theme->get('namespace')]);
        if ($namespace) {
            ...
            $lexicons = $this->modx->getIterator(modLexiconEntry::class, [
                'namespace' => $namespace->name,
