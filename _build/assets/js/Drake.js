...
class NewDrake {
    ...
    initDrake() {
        ...
        try {
            ...
            this.drake = new Sortable(containers, {
                draggable: '.fred--draggable',
                mirror: {
                    constrainDimensions: false,
                },
            });

            this.drake.on('drag:start', (evt) => {
                if ((evt.data.sourceContainer.dataset.fredDropzone !== undefined) && (evt.data.sourceContainer.dataset.fredDropzone !== '')) {
                    if (!evt.data.sensorEvent.data.target.classList.contains('handle')) {
                        evt.cancel();
                    }
                }
        ...
        this.drake.on('drag:stop', (evt) => {
            console.log('drop', evt);
            const target = evt.data.originalSource.parentElement;
            const source = evt.data.sourceContainer;
            const el = evt.data.originalSource;
            const sibling = evt.data.originalSource.nextElementSibling;
    ...
    reloadContainers() {
        this.drake.removeContainer(...this.drake.containers);
        ...
        this.drake.addContainer(...containers);

        console.log('containers', {containers: this.drake.containers});
    }
}


// const drake = new Drake();
const drake = new NewDrake();
